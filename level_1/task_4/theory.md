...
Умение управлять файлами и каталогами — ключевой навык для любого DevOps-инженера или системного администратора. Каждый день вы создаёте новые файлы, копируете конфигурации, перемещаете данные и очищаете ненужные папки. Эти действия кажутся простыми, но именно они лежат в основе работы с серверами, Docker-контейнерами и автоматизацией с помощью Bash-скриптов.

Файлы могут хранить текст, программы, конфигурации или другие данные, а каталоги помогают структурировать и организовывать их. В Linux и macOS используется иерархическая файловая система с корнем `/`, а в Windows файлы распределяются по дискам (`C:\`, `D:\` и т.д.). При работе в WSL структура каталогов полностью повторяет Linux, что позволяет применять одни и те же команды и скрипты на обеих системах.
